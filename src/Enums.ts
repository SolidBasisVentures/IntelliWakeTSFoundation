 ...
 */
export function EnumKeyNames<Enum>(e: Enum): Array<{key: Enum[EnumKeys<Enum>]; name: EnumKeys<Enum>}> {
	return Object.keys(e as any).map((key) => ({
		key: e[key as EnumKeys<Enum>],
		name: key as EnumKeys<Enum>
	}))
}
...
export function EnumValidValue<Enum extends Record<string, number | string>>(
	e: Enum,
	value: Enum[EnumKeys<Enum>] | string | null | undefined | unknown,
	options?: {
		ignoreSpace?: boolean
	}
): boolean {
