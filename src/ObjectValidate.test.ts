...
test('ObjectValidate', () => {
	expect(
		Validator(
			...
			},
			{
				id: [
					{
						[EDefaultCheck.Required]: 'Must have an ID',
						[EDefaultCheck.AtLeast1Character]: 'Must have 1 char'
					},
					(_, vals) => (CleanNumber(vals.id) < 1 ? `Must be greater than 1` : '')
				],
				name: [
					{
						[EDefaultCheck.Required]: 'Must have a name',
				...
				salary: [
					{
						[EDefaultCheck.Required]: 'Must have salary',
						[EDefaultCheck.GreaterThan0]: 'Salary must be greater than 0'
					}
				]
			}
		)
	 ...
	 */ expect(
		...
		id: ['Must have an ID', 'Must have 1 char', 'Must be greater than 1'],
		name: ['Must have a name', 'Must have 1 char']
	})
	...
	expect(
		Validator(
			{id: '2', name: 'John Doe', email: 'John@thisemail.com', hire_date: '2021-11-12', salary: '100000'},
			{
				id: [
					{
						[EDefaultCheck.Required]: 'Must have an ID',
	...
	expect(
		...
		future_date: ['Not a past date'],
		past_date: ['Not a future date']
	})
	...
	expect(
		Validator(
			...
			{
				email: {[EDefaultCheck.IsEmail]: 'Not an e-mail'}
			}
		)
	).toEqual({email: ['Not an e-mail']})
	...
	).toEqual({past_date: ['Not a past date']})
	...
	expect(
		Validator(
			...
			{
				truthy: {[EDefaultCheck.Required]: 'Truthy not available'},
	...
	expect(
		Validator(
			...
			{
				truthy: {[EDefaultCheck.Truthy]: 'Truthy not available'},
				falsey: {[EDefaultCheck.Truthy]: 'Falsey not available'}
			}
		)
	).toEqual({
		falsey: ['Falsey not available']
	})
